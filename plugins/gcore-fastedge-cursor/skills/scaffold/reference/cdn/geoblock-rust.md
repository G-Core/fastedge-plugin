...
If both `BLACKLIST_TW_START` and `BLACKLIST_TW_END` are set, the block only applies during that time window (`now >= tw_start && now <= tw_end`). If neither is set, the block is permanent for all listed countries.
