...
Decodes an `ArrayBuffer` to a UTF-8 string using `TextDecoder`. Returns `''` if `arrVal` is `null`.
...
- Empty string values for required query parameters are treated as missing — the validation rejects them with the same error as a missing parameter.
