...
Note: The workspace-level `[workspace]` key appears in the source Cargo.toml; omit it when embedding as a single-crate project.
...
Log level is set to `Info` at startup.
