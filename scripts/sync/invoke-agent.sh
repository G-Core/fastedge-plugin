...
#              call OpenAI gpt-4o via REST API, parse VERDICT/FINDINGS, write output file.
...
# load_source_files <dir> [file-list]
# When file-list is empty: concatenates every file in <dir> to stdout (original behavior).
# When file-list is provided (newline-separated paths relative to dir): loads only those files.
# Each file is preceded by a filename header.
load_source_files() {
  local dir="$1"
  local file_list="${2:-}"
  ...
  _load_one_file() {
    local fpath="$1" display_name="$2"
    # Skip binary files — null bytes corrupt the prompt and bash command substitution.
    if grep -qP '\x00' "$fpath" 2>/dev/null; then
      echo "WARN: Skipping binary file: ${display_name}" >&2
      return
    fi
  ...
  if [[ -n "$file_list" ]]; then
    ...
    while IFS= read -r relpath; do
      ...
      if [[ "$relpath" == /* ]] || [[ "$relpath" =~ (^|/)\.\.(/|$) ]] || [[ "$relpath" =~ (^|/)\.git(/|$) ]]; then
        echo "ERROR: Refusing unsafe source path: ${relpath}" >&2
        return 1
      fi
      local fpath="${dir}/${relpath}"
      if [[ ! -f "$fpath" ]]; then
        echo "WARN: Source file not found: ${relpath} (in ${dir})" >&2
        continue
      fi
      _load_one_file "$fpath" "$relpath"
    done <<< "$file_list"
  else
    # Load all files in directory (original behavior)
    while IFS= read -r -d '' fpath; do
      _load_one_file "$fpath" "${fpath#"$dir"/}"
    done < <(find "$dir" -type f -not -path "*/.git/*" -print0 | sort -z)
  fi
...
parse_verdict() {
  ...
  local verdict
  verdict=$(grep -i "^VERDICT:" <<< "$response" | head -1 | \
...
_build_generator_prompt() {
  ...
  if [[ -n "$section" ]]; then
    ...
    frontmatter_instruction="⚠️ CRITICAL — Traceability frontmatter: The very first content in the output file MUST be this exact frontmatter block (fill in the values). Do NOT omit it. Do NOT move it. It must appear before any headings or body content:
<!--
  auto-updated: true
  sources:
    - id: ${repo_id}
...
5. **Cross-references**: Do NOT include markdown links to other source doc files (e.g. \`[BUILD_CLI](BUILD_CLI.md)\`). Instead, reference related topics by name in plain text (e.g. "see the BUILD_CLI reference") or collect them in a "See Also" section as a plain-text list of topic names without links. The consuming agents handle their own reference file discovery.
6. **Frontmatter preservation**: YAML frontmatter fields (especially \`capabilities\`, \`type\`, \`app_type\`, \`languages\`) are structural metadata used for blueprint matching. Never drop values from list fields unless the source material explicitly contradicts them.
${section_instruction}
${frontmatter_instruction}
${synthesis_block}
**Output**: Return ONLY the document content as plain text in this response. Do NOT use any file-writing tools (Write, Edit, or similar). Do NOT include preamble, explanations, change summaries, or commentary of any kind.
PROMPT
}
...
_run_generator() {
  ...
  local result
  result=$(claude -p "$(cat "$prompt_file")" --output-format json --allowedTools "" 2>/dev/null \
    | jq -r '.result // empty') || {
  ...
  if [[ -z "$SECTION" ]]; then
    if ! grep -q "auto-updated: true" <<< "$result"; then
      echo "ERROR: Generator output missing traceability frontmatter (auto-updated: true)" >&2
      echo "DEBUG: First 500 chars of Claude output:" >&2
      printf '%s' "${result:0:500}" >&2
      echo "" >&2
      exit 1
    fi
  fi
...
Note: Reference documents are part of a Claude Code plugin and may contain cross-references that are internal to the plugin itself. Plugin skill references (e.g. `/gcore-fastedge:test`) and relative paths to sibling reference files (e.g. `./test-config.md`) are valid plugin-internal links — do NOT flag these as unverifiable against the source repo material. Only flag content that is supposed to reflect the source repo but does not.
...
_run_reviewer() {
  ...
  echo "INFO: Invoking OpenAI gpt-4o reviewer" >&2

  if [[ -z "${OPENAI_API_KEY:-}" ]]; then
    echo "ERROR: OPENAI_API_KEY is not set. Required for the reviewer role." >&2
    exit 1
  fi

  # Call OpenAI gpt-4o via chat completions API
  local request_body
